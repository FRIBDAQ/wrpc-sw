...
void generic_board_i2c_storage(void)
{
	...
	/* EEPROM support.
	   FMC_EEPROM_ADR must be defined in board.h */
...
void generic_board_spi_storage(void)
{
	...
	/*
	 * Gateware has two generics to set sdbfs base address (offset in
	 * flash) and flash sector size.
	 */
	get_storage_info(&sdbfs_entry, &sector_size);
...
int generic_board_storage_init(void)
{
	if (HAS_W1_EEPROM
		...
		generic_board_i2c_storage();
	} else {
		generic_board_spi_storage();
	}
